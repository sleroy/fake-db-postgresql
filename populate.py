...
def get_or_create_countries(engine, num_records: int) -> List[int]:
    ...
    with engine.connect() as conn:
        ...
        if not country_ids:
            # Create minimum required countries
            countries = []
            for _ in range(min(num_records, 100)):  # Limit to 100 countries
                countries.append({
...
def get_or_create_cities(engine, num_records: int, country_ids: List[int]) -> List[int]:
    ...
    with engine.connect() as conn:
        ...
        if not city_ids:
            # Create minimum required cities
            cities = []
            for _ in range(min(num_records, 600)):  # Limit to 600 cities
                cities.append({
                    'city': fake.city(),
...
def get_or_create_addresses(engine, num_records: int, city_ids: List[int]) -> List[int]:
    ...
    with engine.connect() as conn:
        ...
        if not address_ids or len(address_ids) < 10:  # Ensure we have at least 10 addresses
            # Create minimum required addresses
            addresses = []
            for _ in range(max(10, min(num_records, 1000))):
                addresses.append({
            ...
            # Get all address IDs including newly created ones
            result = conn.execute(text("SELECT address_id FROM address"))
            address_ids = [row[0] for row in result]
...
def get_or_create_staff(engine, address_ids: List[int]) -> List[int]:
    ...
    with engine.connect() as conn:
        ...
        if not staff_data or len(staff_data) < 2:  # Ensure we have at least 2 staff members
            # Create minimum required staff (2 members)
            staff = []
            for i in range(2):
                staff.append({
...
def get_or_create_stores(engine, staff_ids: List[int], address_ids: List[int]) -> List[int]:
    ...
    with engine.connect() as conn:
        ...
        if not store_ids:
            ...
            # Create store 2 if we have enough staff
            if len(staff_ids) >= 2:
                conn.execute(text("""
                    INSERT INTO store (store_id, manager_staff_id, address_id, last_update)
...
def create_films(num_records: int, language_ids: List[int]) -> List[dict]:
    """Create new films with valid language IDs"""
    ...
    for _ in range(num_records):
        films.append({
            ...
            'language_id': random.choice(language_ids),  # Use actual language IDs
...
def bulk_insert_data(num_records: int, host: str, user: str, password: str, database: str):
    ...
    language_ids = get_or_create_languages(engine)  # Add this line
    ...
    # Create and insert new films with valid language IDs
    films = create_films(num_records, language_ids)  # Pass language_ids
